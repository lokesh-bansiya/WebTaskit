...
const AdminSideSingleUsersEvents = () => {
    ...
    return (
        <Box>
            {
                events.length === 0 ? (
                    <Box paddingTop="5%" color='red' fontWeight='bold' fontSize={{base: "70%", sm:"70%",md: "80%",lg:"100%",xl:"120%"}}>User dont have any events yet!</Box>
                ) : (
                    <Box width="90%" margin='auto' display='grid' gridTemplateColumns={{base:"repeat(4,1fr)"}}>
                        {
                            events.filter((elem) => elem.userID === userEmail).map((item) => {
                                return (
